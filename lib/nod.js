...
function hasPermission(resourceId, permission){
  ...
  return resource && resource.permissions && resource.permissions[permission];
}
...
exports.grant = function grant(subjectId, resourceId, permission){
  ...
  if(!resource.permissions) resource.permissions = {};

  if(!resource.permissions[permission]) resource.permissions[permission] = [];

  resource.permissions[permission].push(subjectId);
  map[resourceId] = resource;
}

exports.revoke = function revoke(subjectId, resourceId, permission){
  if(hasPermission(resourceId, permission)){
    map[resourceId].permissions[permission] = _.without(map[resourceId].permissions[permission], subjectId);
  }
}
...
function check(subjectId, resourceId, permission){
  // default false
  if(hasPermission(resourceId, permission)){
    return map[resourceId].permissions[permission].indexOf(subjectId) != -1;
  }
  return false;
}
exports.check = check;
